...
def generate_report(idx):
    ...
    workbook = xlsxwriter.Workbook(f'./costos_test/reportes/{idx}.xlsx')
...
def generate_speech(source, sheet, report, campaign):
    ...
    for i in range(0, len(excel[:100])):
        ...
        if i % 10 == 0 and i != 0 or i == len(excel[:100]) - 1:
...
generate_speech('maestria', 'LIDIA', 'Reporte_Mestrias', 'mestrias')
