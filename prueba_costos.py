...
def generate_speech(source, sheet, report, campaign):
    ...
    for i in range(0, len(excel)):
        ...
        if i % 100 == 0 and not iteration_group == 0:
            # Declarar excels
            iteration_group += 1
            ...
            print('Creando carpeta', iteration_group, '...')
            for j in range(0, len(users)):
                if j % 100 == 0:
                    print('Creando audio', j, 'de la carpeta', iteration_group)
                    users[j].create_audio()
                    users[j].add_silence()
                    users[j].generate_details()
                    users[j].generate_report() 
        ...
        name = excel['NOMBRE'][i]
        users.append(Speech( uuid_id, phone, dni, name, engine, idx=validIdx, worksheet=wsh_success, campaign=campaign))
...
def generate_report(idx):
    ...
    # Create the excel report and adding headers
    print('Creating the excel report...')
    workbook = xlsxwriter.Workbook(f'./reportes/{idx}.xlsx')
    worksheet = workbook.add_worksheet('REPORTE')
    worksheet.write('A1', 'ID')
    worksheet.write('B1', 'DNI')
    worksheet.write('C1', 'NÚMERO_TELEFÓNICO')
    ...
    # Create the excel report for the errors
    os.makedirs(f'./reportes/{idx}', exist_ok=True)
    print('Creating the excel report for the errors...')
    workbookError = xlsxwriter.Workbook(f'./reportes/{idx}/errores_{idx}.xlsx')
    worksheetErrores = workbookError.add_worksheet('ERRORES')
    worksheetErrores.write('A1', 'ID')
    worksheetErrores.write('B1', 'DNI')
    worksheetErrores.write('C1', 'NÚMERO_TELEFÓNICO')
